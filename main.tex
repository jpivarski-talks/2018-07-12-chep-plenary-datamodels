...
\node[rectangle callout, draw, inner sep=5 pt, callout relative pointer={(200:1 cm)}, text width=0.43\linewidth] {\large But Spark/Parquet/Arrow/Pandas has nested objects!};
\end{tikzpicture} \hfill \uncover<2->{\begin{tikzpicture}[>=latex]
\node[rectangle callout, draw, inner sep=5 pt, callout relative pointer={(-200:-1 cm)}, text width=0.45\linewidth] {\large Nested data are in these projects' scope, but as second-class citizens.};
\end{tikzpicture}}
...
\large
\vspace{0.7 cm}
\uncover<3->{Also, this {\it should} be of wider interest than HEP: I'm in conversations with developers of Arrow, Dask, and XND ($\sim$Numpy 2.0) about it.}
\end{frame}
...
\vspace{0.75 cm}
\textcolor{blue}{Histograms were Indexed by integers because they didn't have \underline{\it names} back then.}
...
\vspace{0.5 cm}
\uncover<3->{Traditional ansatz fitting, however, optimizes a theory-driven function of few parameters, and the shape of the minimum has implications for the theory.}
...
\item \only<1>{Indexed analysis}\only<2>{\textcolor{darkorange}{\bf Indexed analysis}}\only<3-4>{Indexed analysis}
\item \only<1-2>{Machine learning}\only<3>{\textcolor{darkorange}{\bf Machine learning}}\only<4>{Machine learning}
\end{enumerate}
...
\vspace{0.25 cm}
\begin{enumerate}
\item \only<1-2>{Nested data structures}\only<3>{\textcolor{darkorange}{\bf Nested data structures}}\only<4>{Nested data structures}
\item \only<1>{Advanced histogramming}\only<2>{\textcolor{darkorange}{\bf Advanced histogramming}}\only<3-4>{Advanced histogramming}
...
\large
\vspace{1 cm}
\only<1>{\mbox{ }\vspace{3\baselineskip}}\only<2>{\textcolor{darkorange}{\bf F.A.S.T.\ and histbook are incorporating Pandas indexing into advanced histogramming.}\vspace{2\baselineskip}}\only<3>{\textcolor{darkorange}{\bf Nearly all ML techniques require flattened or sequences of flattened data.} \\ For instance, to train on $N_i$ jets in each event $i$ (nested, unordered sets), we either truncate the number of jets or use RNNs or LSTMs (for non-nested, ordered sequences). {\it Surely it would help to train on the right data type!}}\only<4>{\textcolor{darkorange}{\bf As fits get bigger, they may need to be distributed, for instance with iterative map-reduce.}\vspace{2\baselineskip}}
