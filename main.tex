...
\large
\vspace{1 cm}
\only<1>{\vspace{4\baselineskip}}\only<2>{\textcolor{red}{F.A.S.T.\ and histbook are incorporating Pandas indexing into advanced histogramming.}\vspace{2\baselineskip}}\only<3>{\textcolor{red}{Nearly all ML techniques require flattened or sequences of flattened data.} \\ For instance, to train on $N_i$ jets in each event $i$ (nested, unordered sets), we either truncate the number of jets or use RNNs or LSTMs (for non-nested, ordered sequences).}\only<4>{\textcolor{red}{As fits get bigger, they may need to be distributed, for instance in iterative map-reduce.}\vspace{2\baselineskip}}
