...
\vspace{0.25 cm}
Many physicists consider TTrees with {\tt\small std::vector<float>} branches ``minimal'' or ``flat.''
\end{uncoverenv}
...
\item<5-> \textcolor{darkblue}{HDF5} has lists of compounds, but these are rowwise and specialized.
...
\vspace{-1 cm}
My primary research: object access to columnar data.
...
def mapfcn(event):
    for jet in event.jets:
        yield event.phi - jet.phi
\end{minted}

\column{0.45\linewidth}
\begin{minted}{python}
event["phi"] - event["jet"]["phi"]
\end{minted}
